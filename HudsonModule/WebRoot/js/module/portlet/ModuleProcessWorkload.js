 ...
 * 模具加工单位工作量统计
 */
Ext.define('Module.portlet.ModuleProcessWorkload', {
	...
	initComponent : function() {
		...
		Ext.applyIf(me, {
			// autoScroll : true,
			html : '<div id="workload-parent" style="width:100%;height:100%"><div id="workload-container"></div></div>',

			listeners : {
				render : me.showChart,
				resize : function() {

					var parent = $('#workload-parent');
					var con = $('#workload-container');
					con.width(parent.width());
					con.height(parent.height());
					// 调整大小后重绘
					try {
						me.chart.reflow();
					} catch (e) {
	...
	renderChart : function(me) {
		...
		var options = {
			chart : {
				renderTo : 'workload-container',
				zoomType : 'x',
				spacingRight : 20
			},
			title : {
				text : '<span id="workload-title" >工作量</span><span style="float:right;" id="portlet-craft-select-panel"></span>',
				style : {
					color : '#CD3333',
					fontSize : '12px',
					fontWeight : 'bold'
		...
		Ext.create('Ext.container.Container', {
			renderTo : 'portlet-craft-select-panel',
			width : 200,
			layout : 'hbox',
	...
	getName : function() {
		return Module.portlet.ModuleProcessWorkload.getName();
	}
});
...
Ext.define('PortletCraftMenu', {
	...
	onClickModuleCraft : function() {
		...
		if (mainPanel.date) {
			...
			$('#workload-title').text(this.text + "工作量");
		} else {
			Fly.msg('信息', '您没有选择月份!');
		}
	}
