 ...
 * 
 */
Ext.define('Module.ProcessPartCode', {
		...
		Ext.applyIf(me, {
			items : [ {
				...
				items : [ {
					...
					html : '<div id="process-part-timeline-visualization" style="margin:3px;width:100%;height:200%;"></div>',
					autoScroll : true,
					dockedItems : [ {
						xtype : 'toolbar',
						items : [ {
							text : '查询条件',
							componentCls : 'module-process-part-panel-query',
					...
					listeners : {
						render : function(panel, eo) {
							...
							// 时间轴
							var now = new Date();
							me.timeline = new vis.Timeline(document.getElementById('process-part-timeline-visualization'));
							...
							// 弹出式查询
							$('.module-process-part-panel-query').qtip({
								content : {
									text : '<div id="module-process-part-qtip-panel" style="display:block;width:300px;height:400px;"></div>'
								// title : '查询条件'
								},
								position : {
								...
								hide : {
									...
									effect : function(offset) {
										$(this).slideDown(100); // "this" refers
										// to
										// the tooltip
									}
								},
								show : {
									...
									effect : function(offset) {
										...
										if ($('#module-process-part-qtip-panel').html() == '') {
	 ...
	 * 查询当前加工单位的工件加工时间轴数据
	 */
	onFindDeptPartTimeline : function(moduleCode, moduleState) {
		var me = this;
		App.Progress('工件信息读取中,请稍候...', '工件');

		Ext.Ajax.request({
			url : 'module/part/queryPartTimeline',
			params : {
...
Ext.define('PartQueryQtipPanel', {
	...
	title : '条件选择',
	renderTo : 'module-process-part-qtip-panel',
	...
	initComponent : function() {
		...
		Ext.applyIf(me, {
			items : [ {
				...
				items : [ {
					xtype : 'radiogroup',
					id : 'module-process-state-radiogroup',
					layout : {
						align : 'stretch',
						type : 'hbox'
					},
	...
	onClickSelectModule : function() {
		...
		for (var i = 0; i < models.length; i++) {
			moduleCodes = moduleCodes.concat(models[i].data.modulecode).concat(';');
		}
		var me = this;

		me.parent.onFindDeptPartTimeline(moduleCodes, me.getSelectState());
	},
	...
	getSelectState : function() {
		var group = Ext.getCmp('module-process-state-radiogroup');
		var moduleState = "";
