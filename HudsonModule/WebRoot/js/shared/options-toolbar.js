(function() {
	...
	var scriptTags = document.getElementsByTagName('script'), defaultTheme = 'neptune', defaultRtl = false, i = scriptTags.length, requires = [
			'Ext.toolbar.Toolbar', 'Ext.form.field.ComboBox', 'Ext.form.FieldContainer', 'Ext.form.field.Radio'
	...
	Ext.themeName = theme = Cookies.getCookie('theme') ? Cookies.getCookie('theme') : getQueryParam('theme');
	...
	Ext.onReady(function() {
		...
		function align() {
			...
			try {
				toolbar.alignTo(element, 'tr-tr', [ -5, theme === 'neptune' ? -35 : -24 ]);
			} catch (e) {
				...
				items : [ {
					...
					listeners : {
						select : function(combo) {
							...
							Cookies.setCookie('theme', theme, 9999);
