 ...
 * @extends Ext.dd.DropTarget Internal class that manages drag/drop for
 *          {@link Ext.app.PortalPanel}.
 */
Ext.define('Portlet.PortalDropZone', {
	extend : 'Ext.dd.DropTarget',

	constructor : function(portal, cfg) {
		this.portal = portal;
		Ext.dd.ScrollManager.register(portal.body);
		Portlet.PortalDropZone.superclass.constructor.call(this, portal.body, cfg);
	...
	notifyOver : function(dd, e, data) {
		...
		// handle case scroll where scrollbars appear during
		// drag
		...
		if (!this.lastCW) {
			...
			// client width has changed, so refresh layout &
			// grid calcs
			this.lastCW = cw;
			// portal.doLayout();
			this.grid = this.getGrid();
		...
		// determine column
		var colIndex = 0, colRight = 0, cols = this.grid.columnX;
		var len = cols.length, cmatch = false;
		...
		var overPortlet, pos = 0, h = 0, match = false;
		var overColumn = portal.items.getAt(colIndex);
		var portlets = overColumn.items.items, overSelf = false;
		...
		if (portal.fireEvent('validatedrop', overEvent) !== false && portal.fireEvent('beforedragover', overEvent) !== false) {
			...
			// make sure proxy width is fluid in different width
			// columns
	...
	notifyDrop : function(dd, e, data) {
		...
		var c = this.lastPos.c, col = this.lastPos.col, pos = this.lastPos.p, panel = dd.panel, dropEvent = this.createEvent(dd, e, data, col, c,
		...
		if (this.portal.fireEvent('validatedrop', dropEvent) !== false && this.portal.fireEvent('beforedrop', dropEvent) !== false) {
			...
			// make sure panel is visible prior to inserting so
			// that the layout doesn't ignore it
