...
Ext.define("Sch.locale.En", {
	...
	l10n : {
		...
		"Sch.widget.ExportDialog" : {
			generalError : "一个错误发生,再试一次。",
			title : "导出设置",
			formatFieldLabel : "页面格式",
			orientationFieldLabel : "方向",
			rangeFieldLabel : "导出范围",
			showHeaderLabel : "添加页码",
			orientationPortraitText : "纵向",
			orientationLandscapeText : "模向",
			completeViewText : "完成进度 ",
			currentViewText : "当前查看 ",
			dateRangeText : "日期范围",
			dateRangeFromText : "导出于",
			pickerText : "调整列/行所需的值",
			dateRangeToText : "导出到",
			exportButtonText : "导出",
			cancelButtonText : "取消",
			progressBarText : "导出中...",
			exportToSingleLabel : "导出单独页面",
			adjustCols : "调整列宽",
			adjustColsAndRows : "调整列宽,行高",
...
Ext.define("Sch.util.Patch", {
	...
	onClassExtended : function(a, b) {
		...
		if ((!b.minVersion || Ext.versions.extjs.equals(b.minVersion) || Ext.versions.extjs.isGreaterThan(b.minVersion))
				&& (!b.maxVersion || Ext.versions.extjs.equals(b.maxVersion) || Ext.versions.extjs.isLessThan(b.maxVersion))) {
			if (b.applyFn) {
				b.applyFn()
			} else {
	...
	afterRender : function() {
		...
		if (a) {
			a.getConstrainRegion = function() {
				...
				return d.adjustConstrainRegion(Ext.util.Region.getRegion(b), 0, d.headerCt.forceFit ? (c ? c.getWidth() - d.minColWidth : 0)
						: d.maxColWidth - b.getWidth(), 0, d.minColWidth)
			}
		}
	}
});
...
Ext.define("Sch.util.DragTracker", {
	...
	getRegion : function() {
		var j = this.startXY, f = this.el.getScroll(), l = this.getXY(), c = l[0], b = l[1], h = f.left - this.startScroll.left, m = f.top
				- this.startScroll.top, i = j[0] - h, g = j[1] - m, e = Math.min(i, c), d = Math.min(g, b), a = Math.abs(i - c), k = Math.abs(g - b);
...
Ext.define("Sch.preset.ViewPreset", {
	...
	clone : function() {
		...
		Ext.each([ "rowHeight", "timeColumnWidth", "timeRowHeight", "timeAxisColumnWidth", "displayDateFormat", "shiftUnit", "shiftIncrement",
				"defaultSpan", "timeResolution", "headerConfig" ], function(c) {
			a[c] = b[c]
		});
		...
		weekAndDayLetter : {
			...
			headerConfig : {
				bottom : {
					...
					align : "center",
					renderer : function(a) {						
						return Ext.Date.dayNames[a.getDay()].substring(2, 3)
					}
				},
				middle : {
					unit : "WEEK",
		...
		init : function(a) {
			...
			if (!this.headerTemplate) {
				this.headerTemplate = new Ext.XTemplate('<tpl for=".">', '<div id="{id}" class="{cls}" style="{side}:{position}px;">'
						+ (b ? "{[this.renderInner(values)]}" : "") + "</div>", "</tpl>", {
...
Ext.define("Sch.plugin.Lines", {
	...
	getElementData : function(m, q, c) {
		var t = this.store, j = this.schedulerView, p = j.isHorizontal(), f = c || t.getRange(), h = [], r, a, o = j.getTimeSpanRegion(m, null,
				this.expandToFitView), k, b, e;
		...
		.define("Sch.plugin.Zones",
				{
					...
					init : function(a) {
						...
						if (!this.template) {
							this.template = new Ext.XTemplate('<tpl for="."><div id="{id}" class="{$cls}" style="' + this.side
									+ ':{left}px;top:{top}px;height:{height}px;width:{width}px;{style}">' + (b ? "{[this.renderInner(values)]}" : "")
									+ "</div></tpl>", {
								renderInner : function(c) {
									return b.apply(c)
								}
					...
					getElementData : function(h, d, r, f) {
						var g = this.schedulerView, s = [], c = g.getTimeSpanRegion(h, d, this.expandToFitView), k = this.schedulerView
								.isHorizontal(), b, m, a, j, n, e;
					...
					updateHeaderElement : function(c) {
						var a = this.timeAxis.getStart(), g = this.timeAxis.getEnd(), f = Ext.get(this.getHeaderElementId(c, true)), e = Ext.get(this
								.getHeaderElementId(c, false)), d = this.getZoneHeaderElementData(a, g, c, true), b = this.getZoneHeaderElementData(
								a, g, c, false);
						if (!(f && b) || !(e && b)) {
							Ext.destroy(f, e);
...
Ext
		.define(
				"Sch.tooltip.ClockTemplate",
				{
					...
					constructor : function() {
						var i = Math.PI / 180, l = Math.cos, j = Math.sin, m = 7, c = 2, d = 10, k = 6, f = 3, a = 10, e = Ext.isIE
								&& (Ext.isIE8m || Ext.isIEQuirks);
						function b(n) {
							var q = n * i, o = l(q), t = j(q), r = k * j((90 - n) * i), s = k * l((90 - n) * i), u = Math.min(k, k - r), p = n > 180 ? s
									: 0, v = "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto expand', M11 = " + o + ", M12 = " + (-t)
									+ ", M21 = " + t + ", M22 = " + o + ")";
							return Ext.String.format("filter:{0};-ms-filter:{0};top:{1}px;left:{2}px;", v, u + f, p + a)
						}
						function h(n) {
							var q = n * i, o = l(q), t = j(q), r = m * j((90 - n) * i), s = m * l((90 - n) * i), u = Math.min(m, m - r), p = n > 180 ? s
									: 0, v = "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto expand', M11 = " + o + ", M12 = " + (-t)
									+ ", M21 = " + t + ", M22 = " + o + ")";
...
Ext.define("Sch.tooltip.Tooltip", {
	...
	update : function(a, e, d, f) {
		if (this.startDate - a !== 0 || this.endDate - e !== 0 || this.valid !== d || f) {
			...
			if (this.mode === "calendar" && e.getHours() === 0 && e.getMinutes() === 0
					&& !(e.getYear() === a.getYear() && e.getMonth() === a.getMonth() && e.getDate() === a.getDate())) {
				...
				"Sch.view.HorizontalTimeAxis",
				{
					...
					onElClick : function(a) {
						return function(e, f) {
							...
							var b = Ext.fly(f).getAttribute("headerPosition"), c = Ext.fly(f).getAttribute("headerIndex"), d = this.model
									.getColumnConfig()[b][c];
		...
		.define(
				"Sch.feature.ResizeZone",
				{
					...
					createResizer : function(c, f, p) {
						var m = this.schedulerView, t = this, b = m.getElementFromEventRecord(f), g = m.resolveResource(c), r = m
								.getSnapPixelAmount(), o = m.getScheduleRegion(g, f), q = m.getDateConstraints(g, f), n = c.getHeight, h = (m.rtl && p[0] === "e")
								|| (!m.rtl && p[0] === "w") || p[0] === "n", i = m.getOrientation() === "vertical", e = {
					...
					partialResize : function(b, g, n, l) {
						var q = this.schedulerView, p = l.type === "scroll" ? this.resizer.resizeTracker.lastXY : l.getXY(), o = this
								.getStartEndDates(p), f = o.start, h = o.end, j = b.eventRecord, m = q.isHorizontal();
						if (m) {
					...
					afterResize : function(a, m, f, g) {
						var j = this, i = a.resourceRecord, k = a.eventRecord, d = k.getStartDate(), p = k.getEndDate(), b = a.start || d, c = a.end
								|| p, o = j.schedulerView, n = false, l = true;
						...
						if (b && c && (c - b > 0) && ((b - d !== 0) || (c - p !== 0))
								&& j.validatorFn.call(j.validatorFnScope || j, i, k, b, c, g) !== false) {
		...
		.define(
				"Sch.view.Horizontal",
				{
					...
					getEventRenderData : function(a, b, c) {
						var h = b || a.getStartDate(), g = c || a.getEndDate() || h, j = this.view, f = j.timeAxis.getStart(), k = j.timeAxis
								.getEnd(), i = Math, e = j.getXFromDate(Sch.util.Date.max(h, f)), l = j.getXFromDate(Sch.util.Date.min(g, k)), d = {};
						if (this.view.rtl) {
							d.right = i.min(e, l)
						} else {
							d.left = i.min(e, l)
					...
					getScheduleRegion : function(e, g) {
						var c = Ext.Element.prototype.getRegion ? "getRegion" : "getPageBox", j = this.view, i = e ? Ext.fly(j.getRowNode(e))[c]()
								: j.getTableRegion(), f = j.timeAxis.getStart(), l = j.timeAxis.getEnd(), b = j.getDateConstraints(e, g) || {
							start : f,
							end : l
						}, d = this.translateToPageCoordinate(j.getXFromDate(Sch.util.Date.max(f, b.start))), k = this.translateToPageCoordinate(j
								.getXFromDate(Sch.util.Date.min(l, b.end))), h = i.top + j.barMargin, a = i.bottom - j.barMargin - j.eventBorderWidth;
						return new Ext.util.Region(h, Math.max(d, k), a, Math.min(d, k))
					},
					getResourceRegion : function(j, e, i) {
						var m = this.view, d = m.getRowNode(j), f = Ext.fly(d).getOffsetsTo(m.getEl()), k = m.timeAxis.getStart(), o = m.timeAxis
								.getEnd(), c = e ? Sch.util.Date.max(k, e) : k, g = i ? Sch.util.Date.min(o, i) : o, h = m.getXFromDate(c), n = m
								.getXFromDate(g), l = f[1] + m.cellTopBorderWidth, a = f[1] + Ext.fly(d).getHeight() - m.cellBottomBorderWidth;
						if (!Ext.versions.touch) {
							var b = m.getScroll();
							l += b.top;
							a += b.top
...
Ext.define("Sch.mixin.AbstractTimelineView", {
	...
	getFormattedEndDate : function(c, a) {
		...
		if (c.getHours() === 0 && c.getMinutes() === 0
				&& !(c.getYear() === a.getYear() && c.getMonth() === a.getMonth() && c.getDate() === a.getDate())
				&& !Sch.util.Date.hourInfoRe.test(b.replace(Sch.util.Date.stripEscapeRe, ""))) {
		...
		.define(
				"Sch.mixin.TimelineView",
				{
					...
					scrollElementIntoView : function(b, k, p, f, e) {
						var a = 20, o = b.dom, h = b.getOffsetsTo(k = Ext.getDom(k) || Ext.getBody().dom), d = h[0] + k.scrollLeft, l = h[1]
								+ k.scrollTop, i = l + o.offsetHeight, q = d + o.offsetWidth, m = k.clientHeight, g = parseInt(k.scrollTop, 10), r = parseInt(
								k.scrollLeft, 10), n = g + m, j = r + k.clientWidth, c;
						if (e) {
							if (f) {
								f = Ext.apply({
									listeners : {
...
Ext.define("Sch.mixin.Zoomable", {
	...
	getMilliSecondsPerPixelForZoomLevel : function(c, a) {
		var b = Sch.util.Date;
		return Math.round((b.add(new Date(1, 0, 1), this.getZoomLevelUnit(c), c.increment) - new Date(1, 0, 1))
				/ (a ? c.width : c.actualWidth || c.width))
	},
			...
			return {
				...
				initComponent : function() {
					...
					Ext.Array.forEach([ "eventRendererScope", "eventRenderer", "dndValidatorFn", "resizeValidatorFn", "createValidatorFn",
							...
							"createConfig", "tipCfg", "getDateConstraints" ], function(f) {
						if (f in this) {
							c[f] = this[f]
						}
					}, this);
				...
				"Sch.plugin.Printable",
				{
					...
					mainTpl : new Ext.XTemplate(
									...
									+ 'border-box {htmlClasses}"><head><meta content="text/html; charset=UTF-8" http-equiv="Content-Type" /><title>{title}</title>{styles}</head><body class="sch-print-body {bodyClasses}"><div class="sch-print-ct {componentClasses}" style="width:{totalWidth}px"><div class="sch-print-headerbg" style="border-left-width:{totalWidth}px;height:{headerHeight}px;"></div><div class="sch-print-header-wrap">{[this.printLockedHeader(values)]}{[this.printNormalHeader(values)]}</div>{[this.printLockedGrid(values)]}{[this.printNormalGrid(values)]}</div><script type="text/javascript">{setupScript}<\/script></body></html>',
							{
								printLockedHeader : function(a) {
									var b = "";
									if (a.lockedGrid) {
										b += '<div style="left:-' + a.lockedScroll + "px;margin-right:-" + a.lockedScroll + "px;width:"
								...
								printNormalHeader : function(a) {
									var b = "";
									if (a.normalGrid) {
										b += '<div style="left:' + (a.lockedGrid ? a.lockedWidth : "0") + "px;width:" + a.normalWidth
												+ 'px;" class="sch-print-normalheader ' + a.normalGrid.headerCt.el.dom.className + '">';
								...
								printLockedGrid : function(a) {
									var b = "";
									if (a.lockedGrid) {
										b += '<div id="lockedRowsCt" style="left:-' + a.lockedScroll + "px;margin-right:-" + a.lockedScroll
												+ "px;width:" + (a.lockedWidth + a.lockedScroll) + "px;top:" + a.headerHeight
												+ 'px;" class="sch-print-locked-rows-ct ' + a.innerLockedClasses + " " + Ext.baseCSSPrefix
												+ 'grid-inner-locked">';
										b += a.lockedRows;
										b += "</div>"
									}
								...
								printNormalGrid : function(a) {
									var b = "";
									if (a.normalGrid) {
										b += '<div id="normalRowsCt" style="left:' + (a.lockedGrid ? a.lockedWidth : "0") + "px;top:"
												+ a.headerHeight + "px;width:" + a.normalWidth + 'px" class="sch-print-normal-rows-ct '
												+ a.innerNormalClasses + '">';
										b += '<div style="position:relative;overflow:visible;margin-left:-' + a.normalScroll + 'px">' + a.normalRows
												+ "</div>";
										b += "</div>"
									}
									return b
					...
					print : function() {
						...
						var f = this.getGridContent(g), c = this.mainTpl.apply(Ext.apply({
							...
							setupScript : ("window.onload = function(){ (" + this.setupScript.toString() + ")(" + g.syncRowHeight + ", "
					...
					setupScript : function(e, a, d, b) {
						var c = function() {
							if (e) {
								var f = document.getElementById("lockedRowsCt"), o = document.getElementById("normalRowsCt"), g = f
										&& f.getElementsByTagName("tr"), m = o && o.getElementsByTagName("tr"), k = m && g ? m.length : 0;
								for (var j = 0; j < k; j++) {
		...
		.define(
				"Sch.plugin.Export",
				{
					...
					doExport : function(n, j, q) {
						this.mask();
						var K = this, p = K.scheduler, r = p.getSchedulingView(), m = K.getStylesheets(), I = n || K.defaultConfig, s = p.normalGrid, F = p.lockedGrid, A = s.headerCt
								.getHeight();
						K.saveRestoreData();
						s.expand();
						F.expand();
					...
					getRealSize : function() {
						var c = this.scheduler, b = c.normalGrid.headerCt.getHeight(), e = "." + Ext.baseCSSPrefix
								+ (Ext.versions.extjs.isLessThan("5.0") ? "grid-table" : "grid-item-container"), a = (b + c.lockedGrid.getView()
								.getEl().down(e).getHeight()), d = (c.lockedGrid.headerCt.getEl().first().getWidth() + c.normalGrid.body.down(e)
								.getWidth());
						return {
							width : d,
							height : a
					...
					calculatePages : function(r, s, j, p, b) {
						var t = this, i = t.scheduler, q = i.lockedGrid, c = i.getSchedulingView().timeAxisViewModel.getViewRowHeight(), u = q.headerCt, o = u
								.getEl().first().getWidth(), h = null, k = 0;
						if (o > q.getWidth()) {
					...
					getExportJsonHtml : function(f, E) {
						var H = this, n = H.scheduler, y = [], v = new RegExp(Ext.baseCSSPrefix + "ie\\d?|" + Ext.baseCSSPrefix + "gecko", "g"), B = Ext
								.getBody().dom.className.replace(v, ""), q = n.el.dom.className, m = E.styles, F = E.config, G = E.ticks, o, d, e, p, r;
						if (Ext.isIE) {
							B += " sch-ie-export"
						}
					...
					resizePanelHTML : function(f) {
						...
						if (Ext.isIE6 || Ext.isIE7 || Ext.isIEQuirks) {
							...
							g = d.view.el;
							e = [ k.select("#" + j.id + "-targetEl").first(), k.select("#" + j.id + "-innerCt").first(),
									k.select("#" + d.id).first(), k.select("#" + d.body.id).first(), k.select("#" + g.id) ];
							b = [ k.select("#" + i.id).first(), k.select("#" + i.headerCt.id).first(), k.select("#" + i.body.id).first(),
									k.select("#" + i.getView().id).first() ];
				...
				"Sch.widget.ExportDialogForm",
				{
					...
					initComponent : function() {
						...
						Ext.apply(this, {
							...
							items : [ a.rangeField, a.resizerHolder, a.datesHolder, a.showHeaderField, a.exportToSingleField, a.formatField,
									a.orientationField, a.progressBar || a.createProgressBar() ]
						});
						a.callParent(arguments);
					...
					createFields : function() {
						...
						d.orientationField = new Ext.form.field.ComboBox({
							...
							afterSubTpl : new Ext.XTemplate('<span id="sch-exportdialog-imagePortrait" ' + b
									+ '></span><span id="sch-exportdialog-imageLandscape" ' + c + "></span>"),
							name : "orientation",
							displayField : "name",
							valueField : "value",
							queryMode : "local",
...
Ext.define("Sch.widget.ExportDialog", {
	...
	constructor : function(a) {
		...
		Ext.Array.forEach([ "generalError", "title", "formatFieldLabel", "orientationFieldLabel", "rangeFieldLabel", "showHeaderLabel",
				...
				"pickerText", "dateRangeToText", "exportButtonText", "cancelButtonText", "progressBarText", "exportToSingleLabel" ], function(b) {
			if (b in a) {
				this[b] = a[b]
			}
		}, this);
...
Ext.define("Gnt.model.CalendarDay", {
	...
	verifyAvailability : function(b) {
		...
		Ext.Array.each(b, function(e) {
			if (e.startTime > e.endTime) {
				throw new Error("Start time " + Ext.Date.format(e.startTime, "H:i") + " is greater than end time "
						+ Ext.Date.format(e.endTime, "H:i"))
			}
		});
	...
	stringifyInterval : function(b) {
		var c = b.startTime;
		var a = b.endTime;
		return this.prependZero(c.getHours()) + ":" + this.prependZero(c.getMinutes()) + "-"
				+ (a.getDate() == 1 ? 24 : this.prependZero(a.getHours())) + ":" + this.prependZero(a.getMinutes())
	},
...
Ext.define("Gnt.model.task.More", {
	...
	getConstrainContext : function(f) {
		...
		var a = Gnt.model.Dependency.Type, c = new Date(0), b = new Date(0), i = this.getProjectCalendar(), h = this.getCalendar(), d;
		...
		Ext.each(g, function(l) {
			...
			if (k) {
				...
				var p = l.getLag() || 0, m = l.getLagUnit(), o = k.getStartDate(), j = k.getEndDate();
				switch (l.getType()) {
				case a.StartToEnd:
	...
	addSuccessor : function(b) {
		var d = this.getTaskStore(), c = this.getDependencyStore();
		b = b || new this.self();
		...
		.define("Gnt.model.Task",
				{
					...
					normalize : function() {
						var d = this.getDuration(), h = this.getDurationUnit(), c = this.getStartDate(), g = this.getEndDate(), f = this
								.getSchedulingMode(), e = this.data, j = this.getTaskStore(true);
						...
						if (g && this.inclusiveEndDate) {
							...
							var a = (l && !Ext.Date.formatContainsHourInfo(l))
									|| (g.getHours() === 0 && g.getMinutes() === 0 && g.getSeconds() === 0 && g.getMilliseconds() === 0);
							if (a) {
								if (Ext.isNumber(d)) {
						...
						var n = this.get(k), i = this.getEffortUnit();
						if (f == "FixedDuration") {
					...
					getCalendar : function(a) {
						return a ? this.getOwnCalendar() : this.getOwnCalendar() || this.parentNode && this.parentNode.getCalendar()
					...
					getEffort : function(a) {
						...
						var c = this.getProjectCalendar(), d = c.convertDurationToMs(b, this.getEffortUnit());
					...
					getCalendarDuration : function(a) {
						return this.getProjectCalendar().convertMSDurationToUnit(this.getEndDate() - this.getStartDate(),
					...
					calculateEndDate : function(a, f, d) {
						...
						if (this.isManuallyScheduled()) {
							...
							if (this.getTaskStore(true) && this.hasResources() && c != "FixedDuration" && c != "DynamicAssignment"
									&& c != "EffortDriven") {
					...
					refreshCalculatedParentNodeData : function() {
						var j = new Date(9999, 0, 0), v = new Date(0);
					...
					getAssignmentFor : function(b) {
						var a = null, c = b instanceof Gnt.model.Resource ? b.getInternalId() : b;
					...
					getDisplayStartDate : function(g, d, e, b, c) {
						var f = this.getEndDate(), a = this.getStartDate();
					...
					if (Ext.getVersion("extjs").isGreaterThan("4.2.0.663")) {
						...
						this.override({
							fireEventArgs : function(d, e) {
								var g = Ext.data.Model.prototype.fireEventArgs, b, f, c;
								if (a[d]) {
...
Ext.define("Gnt.util.Data", {
	...
	applyCloneChanges : function(g, m, j, p) {
		...
		var n = g.getModifiedRecords(), q, d, o, h;
		...
		.define(
				"Gnt.data.DependencyStore",
				{
					...
					onDependecyRemove : function(c, b) {
						var a = this.getTaskStore();
						var e = b.getSourceTask(a), d = b.getTargetTask(a);
						if (e) {
					...
					buildCacheKey : function(f, g, c, e, a) {
						var i = a.fromField || (a.fromField = this.model.prototype.fromField), d = a.toField
								|| (a.toField = this.model.prototype.toField), b = a.ignoreDepKey, h = a.addDepKey;
					...
					successorsHaveTransitiveDependency : function(j, k, e, h, b) {
						...
						for (var f = 0, d = c.successors.length; f < d; f++) {
							...
							if (this.hasTransitiveDependency(j, a, e, h) || this.predecessorsHaveTransitiveDependency(j, a, e, h)
									|| this.successorsHaveTransitiveDependency(j, a, e, h, b)) {
					...
					predecessorsHaveTransitiveDependency : function(j, k, d, h, a) {
						...
						for (var f = 0, c = b.predecessors.length; f < c; f++) {
							...
							if (this.hasTransitiveDependency(e, k, d, h) || this.successorsHaveTransitiveDependency(e, k, d, h)
									|| this.predecessorsHaveTransitiveDependency(e, k, d, h, a)) {
					...
					getCycle : function(a) {
						...
						var f = a.visitedTasks, g = a.ignoreTasks, k = a.path, b = a.task, j = b.getInternalId();
						if (g[j]) {
							return
						}
						k.push(b);
					...
					groupsHasTransitiveDependency : function(h, c, d, u, b) {
						...
						var B = q.fromField || (q.fromField = this.model.prototype.fromField), k = q.toField
								|| (q.toField = this.model.prototype.toField), n = q.ignoreDepKey, a = q.addDepKey;
...
Ext.define("Gnt.data.TaskStore", {
	...
	loadData : function(E, y) {
		var C = this, t = C.getRootNode(), c = y ? y.addRecords : false, v = y ? y.syncStore : false;
		C.suspendAutoSync();
		C.suspendEvents();
		if (!c && t) {
			t.removeAll()
		...
		if (E.length) {
			var f = E.length, e = C.model, o = [], w = (typeof E[0].get === "function"), b, r, p, z, B, D, x, n, h, q;
	...
	sortNewNodesByIndex : function(c) {
		...
		Ext.Array.sort(c, function(e, i) {
			var h = a(e, "index"), g = a(i, "index"), f = a(e, "parentId"), d = a(i, "parentId");
	...
	moveChildren : function(e, d, c, f) {
		...
		var b, h = this.nodeIsChild(e, d), g = f ? !this.selfChildInRecordsData(e.getId(), d.getId(), f) : true, a = c
	...
	fillAssignmentsCache : function() {
		var b = this.getAssignmentStore(), a = {};
	...
	onTaskUpdated : function(c, b, a) {
		...
		if (this.lastTotalTimeSpan) {
			...
			if (f && (f[b.endDateField] - d.end === 0 || f[b.startDateField] - d.start === 0)
					|| (b.getEndDate() > d.end || b.getStartDate() < d.start)) {
				this.lastTotalTimeSpan = null
			}
		}
		...
		.define(
				"Gnt.template.Template",
				{
					...
					constructor : function(a) {
						...
						this
								.callParent([
										'<div class="sch-event-wrap {ctcls} '
												+ Ext.baseCSSPrefix
												+ 'unselectable" style="'
												+ c
												+ ':{offset}px">'
												+ (a.leftLabel ? '<div class="sch-gantt-labelct sch-gantt-labelct-left"><label class="sch-gantt-label sch-gantt-label-left">{leftLabel}</label></div>'
														: "")
												+ (a.rightLabel ? '<div class="sch-gantt-labelct sch-gantt-labelct-right" style="left:{width}px"><label class="sch-gantt-label sch-gantt-label-right">{rightLabel}</label></div>'
														: "")
												+ (a.topLabel ? '<div class="sch-gantt-labelct sch-gantt-labelct-top"><label class="sch-gantt-label sch-gantt-label-top">{topLabel}</label></div>'
														: "")
												+ b
												+ (a.bottomLabel ? '<div class="sch-gantt-labelct sch-gantt-labelct-bottom"><label class="sch-gantt-label sch-gantt-label-bottom">{bottomLabel}</label></div>'
														: "") + "</div>", {
											compiled : true,
											disableFormats : true
										} ])
				...
				"Gnt.template.Task",
				{
					...
					getInnerTpl : function(a) {
						...
						return '<div id="'
								...
								+ ((a.resizeHandles === "both" || a.resizeHandles === "left") ? '<div class="sch-resizable-handle sch-gantt-task-handle sch-resizable-handle-start sch-resizable-handle-west"></div>'
										: "")
								+ this.innerTpl
								+ ((a.resizeHandles === "both" || a.resizeHandles === "right") ? '<div class="sch-resizable-handle sch-gantt-task-handle sch-resizable-handle-end sch-resizable-handle-east"></div>'
										: "")
								+ (a.enableProgressBarResize ? '<div style="' + b + ':{percentDone}%" class="sch-gantt-progressbar-handle"></div>'
										: "") + (a.enableDependencyDragDrop ? this.dependencyTerminalMarkup : "") + "</div>"
					}
				});
Ext
		.define(
				...
				{
					extend : "Gnt.template.Template",
					innerTpl : (Ext.isIE8m || Ext.isIEQuirks ? ('<div style="border-width:{[Math.floor(values.side*0.7)]}px" class="sch-gantt-milestone-diamond-top {cls}" unselectable="on" style="{style}"></div><div style="border-width:{[Math.floor(values.side*0.7)]}px" class="sch-gantt-milestone-diamond-bottom {cls}" unselectable="on" style="{style}"></div>')
							: ('<img style="{[values.print ? "height:" + values.side + "px;border-left-width:" + values.side + "px" : ""]};{style}" src="'
									+ Ext.BLANK_IMAGE_URL + '" class="sch-gantt-milestone-diamond {cls}" unselectable="on"/>')),
					getInnerTpl : function(a) {
						return "<div " + (this.isLegacyIE ? 'style="width:{[Math.floor(values.side*0.7)]}px"' : "") + ' id="' + a.prefix
								+ '{id}" class="sch-gantt-item sch-gantt-milestone-diamond-ct">' + this.innerTpl
								+ (a.enableDependencyDragDrop ? this.dependencyTerminalMarkup : "") + "</div>"
					}
				});
Ext
				...
				"Gnt.template.ParentTask",
				{
					...
					getInnerTpl : function(a) {
						return '<div id="' + a.prefix
								+ '{id}" class="sch-gantt-item sch-gantt-parenttask-bar {cls}" style="width:{width}px; {style}">' + this.innerTpl
								+ (a.enableDependencyDragDrop ? this.dependencyTerminalMarkup : "") + "</div>"
					}
				});
Ext.define("Gnt.Tooltip", {
	...
	initComponent : function() {
		...
		if (this.mode === "startend" && !this.startEndTemplate) {
			this.startEndTemplate = new Ext.Template('<div class="sch-timetipwrap {cls}"><div>' + this.L("startText") + "{startText}</div><div>"
					+ this.L("endText") + "{endText}</div></div>").compile()
		}
		if (this.mode === "duration" && !this.durationTemplate) {
			this.durationTemplate = new Ext.Template('<div class="sch-timetipwrap {cls}">', "<div>" + this.L("startText") + " {startText}</div>",
					"<div>" + this.L("durationText") + " {duration} {unit}</div>", "</div>").compile()
		}
		this.callParent(arguments)
	},
...
Ext.define("Gnt.feature.TaskDragDrop", {
	...
	getDragData : function(i) {
		...
		if (f && !i.getTarget(".sch-gantt-baseline-item")) {
			var c = h.resolveTaskRecord(f), b = c.isMilestone();
			if (h.fireEvent("beforetaskdrag", h, c, i) === false) {
				return null
			}
			var m = i.getXY();
			var a = f.cloneNode(true), k = this.showExactDropPosition ? 0 : h.getSnapPixelAmount(), j = Ext.fly(f).getXY();
	...
	finalize : function(c) {
		var e = this.dragData, b = this.gantt, a = e.record, f = e.start, d = false;
		if (c) {
...
Ext.define("Gnt.feature.DependencyDragZone", {
	...
	updateLineProxy : function(m) {
		var a = this.lineProxyEl, j = m[0] - this.startXY[0] + this.el.dom.scrollLeft - this.startScrollLeft, i = m[1] - this.startXY[1]
				+ this.el.dom.scrollTop - this.startScrollTop, b = Math.max(1, Math.sqrt(Math.pow(j, 2) + Math.pow(i, 2)) - 2), h = Math.atan2(i, j)
				- (Math.PI / 2), e;
		if ((Ext.isIE9m || Ext.isIEQuirks)) {
			var k = Math.cos(h), g = Math.sin(h), l = 'progid:DXImageTransform.Microsoft.Matrix(sizingMethod="auto expand", M11 = ' + k + ", M12 = "
					+ (-g) + ", M21 = " + g + ", M22 = " + k + ")", d, f;
...
Ext.define("Gnt.feature.DependencyDropZone", {
	...
	resolveType : function(a, d) {
		var c = Gnt.model.Dependency.Type, b;
		if (a) {
...
Ext.define("Gnt.feature.DependencyDragDrop", {
	...
	dragZoneConfig : null,
	dropZoneConfig : null,
	toolTipTpl : [ '<div class="sch-dd-dependency">', "<table><tbody>", "<tr>", '<td><span class="sch-dd-dependency-from">{fromLabel}:</span></td>',
			'<td><span class="sch-dd-dependency-from-name">{fromTaskName}</span> - {fromSide}</td>', "</tr>", "<tr>",
			'<td><span class="sch-dd-dependency-to">{toLabel}:</span></td>',
			'<td><span class="sch-dd-dependency-to-name">{toTaskName}</span> - {toSide}</td>', "</tr>", "</tbody></table>", "</div>" ],
...
Ext.define("Gnt.feature.ProgressBarResize", {
	...
	createResizable : function(b, g, d) {
		var i = d.getTarget(), h = this.ganttView.rtl, f = b.up(this.ganttView.eventSelector), j = f.getWidth() - 2, c = j * this.increment / 100;
...
Ext.define("Gnt.feature.TaskResize", {
	...
	onMouseDown : function(f, c) {
		var b = this.ganttView, a = f.getTarget(b.eventSelector), g = b.resolveTaskRecord(a);
		var d = g.isResizable();
	...
	onMouseMove : function(h, m) {
		var f = this.ganttView, j = this.taskRec, i = f.getElementFromEventRecord(j), k = f.rtl, a = this.isStart, d = (k && !a) || (!k && a), c = f
				.getSnapPixelAmount(), b = i.getWidth(), l = i.up(f.getItemSelector()).getRegion();
	...
	partialResize : function(k, j, l, p) {
		var h = this.ganttView, f = k.isWest, o = k.taskRecord, g;
		if (f) {
	...
	afterResize : function(n, m, i, k) {
		...
		var l = n.taskRecord, d = l.getStartDate(), o = l.getEndDate(), b = n.isStart ? n.date : d, c = n.isStart ? o : n.date, j = this.ganttView;
		n.destroy();
...
Ext.define("Gnt.plugin.DependencyEditor", {
	...
	buildFields : function() {
		var b = this, c = Gnt.model.Dependency;
	...
	buildTypeField : function() {
		...
		var a = new Ext.data.ArrayStore({
			...
			data : [ [ c.EndToStart, this.L("endToStartText") ], [ c.StartToStart, this.L("startToStartText") ],
					[ c.EndToEnd, this.L("endToEndText") ], [ c.StartToEnd, this.L("startToEndText") ] ]
		});
		...
		.define("Gnt.plugin.Export",
				{
					...
					getExportJsonHtml : function(e, h) {
						var c = this.scheduler.getSchedulingView(), d = c.dependencyView, a = d.painter.getDependencyTplData(c.dependencyStore
								.getRange()), f = d.lineTpl.apply(a), b = h.config, g;
					...
					resizePanelHTML : function(c) {
						var h = this.callParent(arguments), e = h.select(".sch-dependencyview-ct").first(), g = h.select(
								"." + Ext.baseCSSPrefix + "splitter").first(), b = 0, f = 0, a, d;
						...
						if (!c.singlePageExport) {
							...
							if (a) {
								if (d >= a - 1) {
									var j = d - a + 1;
									b += (j === a - 1) ? c.timeColumnWidth * c.columnsAmountLocked : c.timeColumnWidth * c.columnsAmountLocked
											+ (j - 1) * c.timeColumnWidth * c.columnsAmountNormal
								} else {
									g && g.hide()
								}
							} else {
		...
		.define(
				"Gnt.plugin.Printable",
				{
					...
					getGridContent : function(e) {
						...
						j._print = true;
						var a = this.callParent(arguments), h = j.dependencyView, n = h.painter.getDependencyTplData(j.dependencyStore.getRange()), d = '<div class="'
								+ h.containerEl.dom.className + '">' + h.lineTpl.apply(n) + "</div>", m = a.normalRows;
									...
									if (g < (f - 1)) {
										k = c.getAt(c.findBy(function(l) {
											return l.getTargetId() === (o.getId() || o.internalId)
													&& l.getSourceId() === (i[g + 1].getId() || i[g + 1].internalId)
										}));
		...
		.define(
				"Gnt.view.DependencyPainter",
				{
					...
					getTaskBox : function(s) {
						var i = Sch.util.Date, k = s.getStartDate(), r = s.getEndDate(), o = this.ganttView, n = o.bufferedRenderer, f = o.timeAxis
								.getStart(), d = o.timeAxis.getEnd();
						...
						var g, c = o.getXFromDate(i.max(k, f)), b = o.getXFromDate(i.min(r, d)), a = o.getNodeByRecord(s);
						if (a || n) {
							var v = o.getXOffset(s), p, j, w = s.isMilestone(), u = true;
							if (c > v) {
								c -= v
							}
							b += v;
					...
					getLineCoordinates : function(q, o, j) {
						var f, r, p = [ 0, q.top - 1 + ((q.bottom - q.top) / 2) ], a = [ 0, o.top - 1 + ((o.bottom - o.top) / 2) ], b = a[1] > p[1], g = Gnt.model.Dependency.Type, d = this.arrowOffset
								+ this.xOffset, c = j.getType(), m = [], s = j.getTargetTask().isMilestone(), k, h, t;
						switch (c) {
						case g.StartToEnd:
							p[0] = q.start;
							a[0] = o.end + d;
		...
		.define(
				"Gnt.view.Dependency",
				{
					...
					constructor : function(a) {
							...
							this.lineTpl = Ext
									.create(
											...
											'<tpl for=".">'
													+ Ext.String
															.format(
																	'<tpl for="lineCoordinates"><div class="{0} {[ parent.dependency.isHighlighted ? "{1}" : "" ]} {[values.x1==values.x2 ? "sch-dependency-line-v" : "sch-dependency-line-h"]} {lineCls} sch-dep-{parent.id} {0}-line {[this.getSuffixedCls(parent.cls, "-line")]}" style="'
																			+ b
																			+ ":{[Math.min(values.x1, values.x2)]}px;top:{[Math.min(values.y1, values.y2)]}px;width:{[Math.abs(values.x1-values.x2)+"
																			+ this.lineWidth
																			+ "]}px;height:{[Math.abs(values.y1-values.y2)+"
																			+ this.lineWidth
																			+ ']}px"></div></tpl><div style="'
																			+ b
																			+ ':{[values.lineCoordinates[values.lineCoordinates.length - 1].x2]}px;top:{[values.lineCoordinates[values.lineCoordinates.length - 1].y2]}px"    class="{0}-arrow-ct {0} {[ values.dependency.isHighlighted ? "{1}" : "" ]} sch-dep-{id} {[this.getSuffixedCls(values.cls, "-arrow-ct")]}"><img src="'
																			+ Ext.BLANK_IMAGE_URL
																			+ '" class="{0}-arrow {0}-arrow-{[this.getArrowDirection(values.lineCoordinates)]} {[this.getSuffixedCls(values.cls, "-arrow")]}" /></div>',
																	this.dependencyCls, this.selectedCls) + "</tpl>", {
												compiled : true,
												disableFormats : true,
						...
						this.addEvents("beforednd", "dndstart", "drop", "afterdnd", "dependencyclick", "dependencycontextmenu", "dependencydblclick",
								"refresh");
...
Ext
		.define(
				"Gnt.view.Gantt",
				{
					...
					requires : [ "Ext.dd.ScrollManager", "Gnt.view.Dependency", "Gnt.model.Task", "Gnt.template.Task", "Gnt.template.ParentTask",
							"Gnt.template.Milestone", "Gnt.feature.TaskDragDrop", "Gnt.feature.ProgressBarResize", "Gnt.feature.TaskResize",
							"Sch.view.Horizontal" ],
					...
					constructor : function(a) {
						...
						this.addEvents("taskclick", "taskdblclick", "taskcontextmenu", "beforetaskresize", "taskresizestart", "partialtaskresize",
								"aftertaskresize", "beforeprogressbarresize", "progressbarresizestart", "afterprogressbarresize", "beforetaskdrag",
								"taskdragstart", "beforetaskdropfinalize", "taskdrop", "aftertaskdrop", "labeledit_beforestartedit",
								"labeledit_beforecomplete", "labeledit_complete", "beforedependencydrag", "dependencydragstart", "dependencydrop",
								"afterdependencydragdrop", "beforedragcreate", "dragcreatestart", "dragcreateend", "afterdragcreate",
					...
					columnRenderer : function(z, u, l) {
						var m = l.getStartDate(), p = this.timeAxis, v = Sch.util.Date, b = {}, G = "", r = "", h = p.getStart(), g = p.getEnd(), J = l
								.isMilestone(), C = l.isLeaf(), q, s, y;
						if (m) {
							var A = l.getEndDate() || Sch.util.Date.add(m, l.getDurationUnit() || Sch.util.Date.DAY, 1), e = Sch.util.Date
									.intersectSpans(m, A, h, g);
							if (e) {
								y = A > g;
								s = v.betweenLesser(m, h, g);
								var F = Math.floor(this.getXFromDate(s ? m : h)), d = Math.floor(this.getXFromDate(y ? g : A)), f = J ? 0 : d - F, w = this.leftLabelField, k = this.rightLabelField, x = this.topLabelField, j = this.bottomLabelField, I = this
										.getTemplateForTask(l);
								if (!J && !C) {
									if (y) {
						...
						if (this.enableBaseline) {
							var t = l.getBaselineStartDate(), a = l.getBaselineEndDate();
							if (!q) {
							...
							if (t && a && Sch.util.Date.intersectSpans(t, a, h, g)) {
								...
								var c = l.isBaselineMilestone(), o = Math.floor(this.getXFromDate(s ? t : h)), n = Math.floor(this.getXFromDate(y ? g
										: a)), E = c ? 0 : n - o, B = this.getTemplateForTask(l, true), H = {
...
Ext.define("Gnt.view.ResourceHistogram", {
	...
	initComponent : function(a) {
		...
		if (!this.barTpl) {
			this.barTpl = new Ext.XTemplate('<tpl for=".">', '<div id="{id}" class="' + this.barCls
					+ ' {cls}" gnt-bar-index="{index}" style="left:{left}px;top:{top}px;height:{height}px;width:{width}px"></div>',
					"<tpl if=\"text !== ''\">", '<span class="' + this.barCls + '-text" style="left:{left}px;">{text}</span>', "</tpl>", "</tpl>")
		}
		if (!this.lineTpl) {
			this.lineTpl = new Ext.XTemplate('<tpl for=".">', '<div class="' + this.lineCls + ' {cls}" style="top:{top}px;"></div>', "</tpl>")
		}
		if (!this.limitLineTpl) {
			this.limitLineTpl = new Ext.XTemplate('<tpl for=".">', '<div class="' + this.limitLineCls
					+ ' {cls}" style="left:{left}px;top:{top}px;width:{width}px;height:{height}px"></div>', "</tpl>")
		}
	...
	prepareLines : function(g) {
		var h = g.scaleMin, d = g.scaleLabelStep, a = this.getAvailableRowHeight(), e = [], m = {}, k = this.lineCls, j = k + "min";
		if (g.scalePoints) {
			var f;
	...
	prepareLimitLines : function(m, g) {
		var k = [], a = this.getAvailableRowHeight(), n = this.limitLineCls, b;
	...
	prepareBars : function(j, e, f) {
		var h = [], a = this.getAvailableRowHeight(), k = this.barCls, b, g;
	...
	columnRenderer : function(e, d, c, g, b) {
		var f = c.getInternalId(), a = this.normalGrid.getView();
		return (this.showScaleLines ? a.renderLines(this) : "") + a.renderBars(this, this.allocationData[f].bars, f)
				+ (this.showLimitLines ? a.renderLimitLines(this, this.allocationData[f].maxBars) : "")
	},
		...
		.define(
				"Gnt.column.Scale",
				{
					...
					initComponent : function() {
						...
						if (!this.tpl) {
							this.tpl = new Ext.XTemplate('<div class="' + this.scaleCellCls + '-wrap" style="height:{scaleHeight}px;">',
									'<tpl for="scalePoints">', "<tpl if=\"label !== ''\">", '<span class="' + this.scaleCellCls
											+ '-label-line {cls}" style="top:{top}px"><span class="' + this.scaleCellCls
											+ '-label">{label}</span></span>', "<tpl else>", '<span class="' + this.scaleCellCls
											+ '-line {cls}" style="top:{top}px"></span>', "</tpl>", "</tpl>", "</div>")
						}
					...
					buildScalePoints : function() {
						var g = this.scaleMin, h = g, c = this.scaleStep, f = this.scaleLabelStep, d = this.scaleStepHeight, b = this.availableHeight, a = this.scaleCellCls, i = a
								+ "-min", j = [];
					...
					updateScalePointsTops : function() {
						var d = this.scaleStepHeight, e = this.availableHeight, a;
...
Ext
		.define(
				"Gnt.panel.Gantt",
				{
					...
					requires : [ "Ext.layout.container.Border", "Gnt.model.Dependency", "Gnt.data.ResourceStore", "Gnt.data.AssignmentStore",
							"Gnt.feature.WorkingTime", "Gnt.data.Calendar", "Gnt.data.TaskStore", "Gnt.data.DependencyStore", "Gnt.view.Gantt" ],
					...
					initComponent : function() {
						...
						this.relayEvents(c, [ "taskclick", "taskdblclick", "taskcontextmenu", "beforetaskresize", "taskresizestart",
								"partialtaskresize", "aftertaskresize", "beforeprogressbarresize", "progressbarresizestart",
								"afterprogressbarresize", "beforetaskdrag", "taskdragstart", "beforetaskdropfinalize", "taskdrop", "aftertaskdrop",
								"labeledit_beforestartedit", "labeledit_beforecomplete", "labeledit_complete", "beforedependencydrag",
								"dependencydragstart", "dependencydrop", "afterdependencydragdrop", "dependencyclick", "dependencycontextmenu",
								"dependencydblclick", "scheduleclick", "scheduledblclick", "schedulecontextmenu" ]);
						if (this.addRowOnTab) {
					...
					onEditorTabPress : function(j, i) {
						var k = this.lockedGrid.view, g = j.getActiveRecord(), d = j.getActiveColumn(), h = k.getPosition(g, d), a = this.lockedGrid.headerCt, f = h.row === this.lockedGrid.view.store
								.getCount() - 1, c = function(e) {
					...
					refreshViews : function() {
						...
						var b = this.lockedGrid.getView(), c = b.el.dom.scrollLeft, a = b.el.dom.scrollTop;
						b.refresh();
...
Ext.define("Gnt.panel.ResourceHistogram", {
	...
	initComponent : function() {
		...
		if (!this.columns) {
			...
			var e = !Ext.isArray(this.columns) ? [ this.columns ] : this.columns, d;
		...
		Ext.Array.forEach([ "barCls", "barTpl", "barRenderer", "lineTpl", "lineCls", "limitLineTpl", "limitLineCls", "limitLineWidth", "labelMode",
				"labelPercentFormat", "labelUnitsFormat", "scaleMin", "scaleMax", "scaleStep", "loadMask" ], function(i) {
			if (i in this) {
	...
	processAllocationData : function(z) {
		var w, x, h, u, o, c, b, m, d, A, r = [], g = [], a = false, y = this;
		var p = function() {
...
Ext.define("Gnt.field.StartDate", {
	...
	onSelect : function(d, a) {
		...
		var e = this, f = Ext.Date.format(a, this.format), b = e.getValue(), g = this.visibleToValue(a), h = this.getErrors(f);
		if (b != g) {
			if (h && h.length) {
				e.setRawValue(f);
				e.collapse();
...
Ext.define("Gnt.field.Duration", {
	...
	valueToVisible : function(c, d) {
		if (Ext.isNumber(c)) {
			...
			return String(b == a ? b : a).replace(".", this.decimalSeparator) + " "
					+ Sch.util.Date[this.useAbbreviation ? "getShortNameOfUnit" : "getReadableNameOfUnit"](d || this.durationUnit, c !== 1)
		}
		return ""
	},
	valueToRaw : function(a) {
...
Ext.define("Gnt.field.SchedulingMode", {
	...
	store : [ [ "Normal", "Normal" ], [ "Manual", "Manual" ], [ "FixedDuration", "Fixed duration" ], [ "EffortDriven", "Effort driven" ],
		...
		.define(
				"Gnt.field.Dependency",
				{
					...
					getErrors : function(n) {
						...
						var g = this.getDependencies(), m = this.isPredecessor(), s = this.task, u = s.getTaskStore().dependencyStore, l = s[m ? "predecessors"
								: "successors"], a;
						var t = [];
						for (var r = 0; r < g.length; r++) {
							...
							if (u.allowedDependencyTypes && !u.isValidDependencyType(p.type)) {
								var h = this.dependencyParser.types, d = u.allowedDependencyTypes, f = u.model.Type, b = "";
					...
					getDisplayValue : function(a) {
						var h = this.isPredecessor(), l = h ? a.getIncomingDependencies(true) : a.getOutgoingDependencies(true), j = Gnt.util.DependencyParser.prototype.types, e = Gnt.model.Dependency.Type.EndToStart, k = [], m;
						for (var d = 0; d < l.length; d++) {
							var g = l[d];
							m = h ? g.getSourceTask() : g.getTargetTask();
							if (m && g.isValid(false)) {
								var f = g.getType(), b = g.getLag(), c = g.getLagUnit();
								k.push(Ext.String.format("{0}{1}{2}{3}{4}", this.useSequenceNumber ? m.getSequenceNumber() : m.getInternalId(), b
										|| f !== e ? j[f] : "", b > 0 ? "+" : "", b || "", b && c !== "d" ? c : ""))
							}
						}
					...
					isDirty : function(c) {
						...
						var a = this.isPredecessor(), d = c.getTaskStore().dependencyStore, h = a ? c.getIncomingDependencies() : c
								.getOutgoingDependencies(), f = c.getInternalId();
					...
					applyChanges : function(d) {
						d = d || this.task;
						var n = d.getTaskStore().dependencyStore, h = this.getDependencies(), a = [], k = this.isPredecessor(), m = k ? d
								.getIncomingDependencies(true) : d.getOutgoingDependencies(true), c = [];
...
Ext.define("Gnt.widget.AssignmentGrid",
		{
			...
			requires : [ "Gnt.model.Resource", "Gnt.model.Assignment", "Gnt.column.ResourceName", "Gnt.column.AssignmentUnits",
...
Ext.define("Gnt.field.Assignment", {
	...
	isDirty : function(c) {
		...
		var e = this.picker && this.picker.assignmentStore || c.getAssignmentStore(), b = c.getAssignments(), f;
...
Ext.define("Gnt.column.Name", {
	...
	afterRender : function() {
		var b = this, a = this.up("treepanel");
		if (!this.dataIndex) {
...
Ext.define("Gnt.column.Slack", {
	...
	rendererFunc : function(b, c, a) {
		...
		b = a.getSlack();
		if (Ext.isNumber(b)) {
			return parseFloat(Ext.Number.toFixed(b, this.decimalPrecision)) + " "
					+ Sch.util.Date[this.useAbbreviation ? "getShortNameOfUnit" : "getReadableNameOfUnit"](this.slackUnit, b !== 1)
		}
		return ""
	}
});
...
Ext.define("Gnt.widget.AssignmentEditGrid", {
	...
	requires : [ "Ext.data.JsonStore", "Ext.window.MessageBox", "Ext.form.field.ComboBox", "Ext.grid.plugin.CellEditing", "Gnt.util.Data",
			"Gnt.data.AssignmentStore", "Gnt.data.ResourceStore", "Gnt.model.Resource", "Gnt.model.Assignment", "Gnt.column.ResourceName",
			"Gnt.column.AssignmentUnits" ],
	mixins : [ "Gnt.mixin.Localizable" ],
	assignmentStore : null,
	resourceStore : null,
	...
	loadTaskAssignments : function(d) {
		...
		var c = this.taskStore || this.assignmentStore.getTaskStore(), b = c && c.getByInternalId(d), g;
		if (b) {
			g = b.assignments
		} else {
			if (!this.assignmentStore) {
	...
	filterResources : function(c) {
		var d = c.getInternalId(), b = Gnt.model.Assignment.prototype.resourceIdField, a = true;
	...
	onResourceComboAssert : function(f) {
		...
		if (e) {
			...
			if (!b) {
				...
				var d = this;
				var c = function(j) {
					var k = Gnt.model.Resource.prototype, i = {};
...
Ext.define("Gnt.widget.DependencyGrid", {
	...
	requires : [ "Ext.data.JsonStore", "Ext.grid.plugin.CellEditing", "Ext.form.field.ComboBox", "Ext.util.Filter", "Gnt.model.Dependency",
			"Gnt.util.Data", "Gnt.field.Duration" ],
	mixins : [ "Gnt.mixin.Localizable" ],
	readOnly : false,
	showCls : false,
	...
	constructor : function(b) {
		b = b || {};
		Ext.Array.each([ "idText", "taskText", "blankTaskText", "invalidDependencyText", "parentChildDependencyText", "duplicatingDependencyText",
				"transitiveDependencyText", "cyclicDependencyText", "typeText", "lagText", "clsText", "endToStartText", "startToStartText",
				"endToEndText", "startToEndText" ], function(c) {
			if (c in b) {
				this[c] = b[c]
			}
		}, this);
	...
	filterTasks : function(a) {
		var c = this, b = a.getInternalId(), d;
	...
	buildTypesCombo : function() {
		...
		var a = new Ext.data.ArrayStore({
			...
			data : [ [ b.EndToStart, this.L("endToStartText") ], [ b.StartToStart, this.L("startToStartText") ],
					[ b.EndToEnd, this.L("endToEndText") ], [ b.StartToEnd, this.L("startToEndText") ] ]
		});
		a.filter(this.typesFilter);
		return new Ext.form.field.ComboBox({
			triggerAction : "all",
	...
	buildColumns : function() {
		var d = this, b = this.store.model.prototype, f = this.store.model.Type, e = [], a = this.dependencyStore
				&& this.dependencyStore.getTaskStore();
		...
		if (this.useSequenceNumber) {
			c = {
				...
				renderer : function(j, k, g) {
					var h = d.dependencyStore && d.dependencyStore.getTaskStore(), i = h && h.getNodeById(g.get("From"));
	...
	insertDependency : function(b, h) {
		...
		var e = this.task.getInternalId(), d = this.store.model.prototype, c = {}, g = this;
		if (b) {
			c = b
		} else {
	...
	getDependencyErrors : function(c) {
		var h = this, j = h.dependencyStore, d = h.task.getInternalId(), b = d, i = [];
...
Ext
		.define(
				"Gnt.widget.taskeditor.TaskForm",
				{
					...
					requires : [ "Gnt.model.Task", "Ext.form.FieldContainer", "Ext.form.field.Text", "Ext.form.field.Date", "Gnt.field.Percent",
							"Gnt.field.StartDate", "Gnt.field.EndDate", "Gnt.field.Duration", "Gnt.field.SchedulingMode", "Gnt.field.Effort" ],
					mixins : [ "Gnt.mixin.Localizable" ],
...
Ext.define("Gnt.widget.taskeditor.TaskEditor", {
	...
	requires : [ "Ext.form.Panel", "Gnt.widget.taskeditor.TaskForm", "Gnt.widget.AssignmentEditGrid", "Gnt.widget.DependencyGrid",
			"Gnt.field.Calendar", "Gnt.field.SchedulingMode", "Ext.form.field.HtmlEditor", "Gnt.util.Data" ],
	mixins : [ "Gnt.mixin.Localizable" ],
	margin : "5 0 0 0",
	...
	clonedStores : null,
	constructor : function(b) {
		var f = this, e = Gnt.model.Task.prototype;
		b = b || {};
		Ext.apply(this, b);
	...
	cloneStores : function(d) {
		var a = this.task, e = this.cloneResourceStore(a, d && d.resourceStore), f = this.cloneAssignmentStore(a, d && d.assignmentStore), c = this
				.cloneDependencyStore(a, d && d.dependencyStore);
	...
	validate : function() {
		var b = this.getActiveTab(), a = true, c;
		if (b) {
...
Ext.define("Gnt.plugin.TaskEditor", {
	...
	init : function(d) {
		...
		var b = {
			...
			height : null,
			border : false
		}, e = [ "l10n", "task", "taskStore", "assignmentStore", "resourceStore", "generalText", "resourcesText", "dependencyText",
				"addDependencyText", "dropDependencyText", "notesText", "advancedText", "wbsCodeText", "addAssignmentText", "dropAssignmentText",
				"showAssignmentGrid", "showDependencyGrid", "allowParentTaskDependencies", "showNotes", "showStyle", "showAdvancedForm",
				"taskFormClass", "advancedFormClass", "tabsConfig", "taskFormConfig", "dependencyGridConfig", "assignmentGridConfig",
				"advancedFormConfig", "styleFormConfig", "dependencyGridClass", "assignmentGridClass" ];
		for (var c = 0, a = e.length; c < a; c++) {
...
Ext
		.define(
				...
				{
					extend : "Ext.form.Panel",
					requires : [ "Ext.XTemplate", "Ext.data.Store", "Ext.grid.Panel", "Ext.grid.plugin.CellEditing", "Gnt.data.Calendar",
							"Gnt.model.CalendarDay", "Gnt.widget.calendar.DayEditor", "Gnt.widget.calendar.WeekEditor",
					...
					setupTemplates : function() {
						...
						this.dateInfoTpl = this.dateInfoTpl
								|| Ext.String
										.format(
												'<tpl if="isWorkingDay == true"><div>{0} {date}:</div></tpl><tpl if="isWorkingDay == false"><div>{date} {1}</div></tpl><ul class="gnt-calendar-availabilities"><tpl for="availability"><li>{.}</li></tpl></ul><span>{5}: <tpl if="override == true">{2} "{name}" {3} "{calendarName}"</tpl><tpl if="override == false">{4} "{calendarName}"</tpl></span>',
												a.tplWorkingHours, a.tplIsNonWorking, a.tplOverride, a.tplInCalendar, a.tplDayInCalendar,
												a.tplBasedOn)
					},
					afterRender : function() {
						this.callParent(arguments);
					...
					editDay : function() {
						var e = this, c = this.getDayGrid().getSelectionModel().getSelection();
						if (c.length === 0) {
							return
						}
						var a = c[0];
					...
					removeDay : function() {
						var a = this.getDayGrid(), b = a.getSelectionModel().getSelection();
						if (!b.length) {
							return
						}
					...
					refreshView : function() {
						var f = this.getDatePicker().getValue(), b = this.getCalendarDay(f), e = this.getWeekGrid(), a = this.getDayGrid(), d = this.dayOverridesCalendar
								.getOwnCalendarDay(f), h;
						var c;
						if (d) {
					...
					editWeek : function() {
						var c = this.getWeekGrid().getSelectionModel().getSelection(), e = this;
						if (c.length === 0) {
							return
						}
						var b = c[0];
					...
					removeWeek : function() {
						var a = this.getWeekGrid().getSelectionModel().getSelection(), b = this;
						if (a.length === 0) {
							return
						}
					...
					onWeekGridValidateEdit : function(d, b) {
						var c = b.record, a = b.field == "startDate" ? b.value : c.get("startDate"), e = b.field == "endDate" ? b.value : c
								.get("endDate");
						if (a > e) {
							this.alert({
