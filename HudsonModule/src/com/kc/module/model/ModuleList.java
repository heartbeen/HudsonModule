...
public class ModuleList extends ModelFinal<ModuleList> {
    ...
    public List<ModuleList> getDeviserModuleInfo(String match, boolean ds) {
        StringBuilder builder = new StringBuilder();
        builder.append("SELECT ML.MODULEBARCODE, FY.SHORTNAME AS GUESTNAME, ML.MODULECLASS,ML.GUESTID, ML.GUESTCODE, ML.PRODUCTNAME, ML.MODULECODE, ML.UNITEXTRAC,");
        ...
        builder.append("DR.MODULEBARCODE LEFT JOIN FACTORY FY ON ML.GUESTID = FY.ID WHERE 1 = 1 ");
        if (!StringUtils.isEmpty(match)) {
